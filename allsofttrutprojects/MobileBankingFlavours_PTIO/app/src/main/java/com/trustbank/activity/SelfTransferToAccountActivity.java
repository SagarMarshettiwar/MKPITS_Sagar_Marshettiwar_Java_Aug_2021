...
public class SelfTransferToAccountActivity extends AppCompatActivity implements View.OnClickListener, AlertDialogOkListener , GetAmountInterface {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setText(R.string.SelfAccountTransfer);
        inIt();
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.backButton_new:
                if(AppConstants.menu_group){
                    Intent intent = new Intent(SelfTransferToAccountActivity.this, MenuActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                    startActivity(intent);
                    AppConstants.menu_group=false;
                    finish();
                    break;
                }else{
                    Intent intent = new Intent(SelfTransferToAccountActivity.this, TransactionMenu.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                    startActivity(intent);
                    finish();
                    break;
            ...
            case R.id.btnWithBankNextId:
                ...
                DialogFragment newFragment = new EnterAmountFragment(SelfTransferToAccountActivity.this,getAmountInterface,toAccountNo.trim());
    ...
    public void getAmount(String amount, String remark) {
        String selectedFromAccount = spinnerFrmAct.getSelectedItem().toString().trim();
        String selectedToAccount = spinnerToAccount.getSelectedItem().toString().trim();
        try {
            TrustMethods.hideSoftKeyboard(SelfTransferToAccountActivity.this);
            if (spinnerFrmAct.getSelectedItem().equals(getResources().getString(R.string.SelectAccountNumber))) {
                TrustMethods.showSnackBarMessage(getResources().getString(R.string.error_select_acc_no), coordinatorLayout);
            } else if (spinnerToAccount.getSelectedItem().equals(getResources().getString(R.string.SelectAccountNumber))) {
                TrustMethods.showSnackBarMessage(getResources().getString(R.string.error_select_to_acc_no), coordinatorLayout);
            } else if (selectedFromAccount.equals(selectedToAccount)) {
                TrustMethods.showSnackBarMessage(getResources().getString(R.string.error_select_from_to), coordinatorLayout);
            }  else {
                String toAccNo = spinnerToAccount.getSelectedItem().toString().trim();
                toAccountNo = TrustMethods.getValidAccountNo(toAccNo);
    ...
    private void checkAmountValidation(String amount, String accountNo, List<FundTransferSubModel> fundTransferSubModalList) {
        if (NetworkUtil.getConnectivityStatus(SelfTransferToAccountActivity.this)) {
            new IMPSAddValidationAsyncTask(SelfTransferToAccountActivity.this, accountNo, amount,fundTransferSubModalList).execute();
        } else {
            ...
            try {
                ...
                if (!this.error.equals("")) {
                    if (TrustMethods.isSessionExpired(errorCode)) {
                        AlertDialogMethod.alertDialogOk(SelfTransferToAccountActivity.this, getResources().getString(R.string.error_session_expire), "",getResources().getString(R.string.btn_ok), 0, false, alertDialogOkListener);
                    } else {
