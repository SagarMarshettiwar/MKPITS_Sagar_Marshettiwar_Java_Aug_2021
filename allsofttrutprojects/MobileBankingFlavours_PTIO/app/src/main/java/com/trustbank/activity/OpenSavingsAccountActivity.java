...
public class OpenSavingsAccountActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    Button btn_submit;
    EditText et_name,et_email,et_address, et_remarks;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (AppConstants.IS_CHECK_ACCESS_BROKEN) {
            if (savedInstanceState != null) {
                ...
                if (currentPID != savedInstanceState.getString(AppConstants.PID)) {
                    ...
                    TrustMethods.naviagteToSplashScreen(OpenSavingsAccountActivity.this);
                }
            }
        }
        SetTheme.changeToTheme(OpenSavingsAccountActivity.this, false);
        ...
        toolbar.setText(R.string.OpenSavingsAccount);
    ...
    private void inIT() {
        ...
        methods=new TrustMethods(OpenSavingsAccountActivity.this);
        ...
        if (TrustMethods.isSimAvailable(getApplicationContext()) && TrustMethods.isSimVerified(OpenSavingsAccountActivity.this)) {
            if (NetworkUtil.getConnectivityStatus(OpenSavingsAccountActivity.this)) {
                new AsyncTaskGetSavinglookups(OpenSavingsAccountActivity.this).execute();
            } else {
                Toast.makeText(this, getResources().getString(R.string.error_check_internet), Toast.LENGTH_SHORT).show();
            }
        } else {
            TrustMethods.displaySimErrorDialog(OpenSavingsAccountActivity.this);
        }
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if(!hasFocus){
                    if(Identification_spi.getSelectedItemPosition()==6 && et_identificationno.length() ==13 ){
                        ...
                        new AsyncTaskverifyIdentificationno(OpenSavingsAccountActivity.this,et_identificationno.getText().toString(),action1).execute();

                    }else if(Identification_spi.getSelectedItemPosition()==1 && et_identificationno.length() ==10){
                        String action="VALIDATE_CEDULA";
                        new AsyncTaskverifyIdentificationno(OpenSavingsAccountActivity.this,et_identificationno.getText().toString(),action).execute();
                    }else{
                        if(Identification_spi.getSelectedItemPosition()==6 && et_identificationno.length() !=13){
                            Toast.makeText(OpenSavingsAccountActivity.this, "Please Enter 13 Digits RUC Number", Toast.LENGTH_SHORT).show();
                        } else if (Identification_spi.getSelectedItemPosition()==1 && et_identificationno.length() !=10) {
                            Toast.makeText(OpenSavingsAccountActivity.this, "Please Enter 10 Digits CEDULA Number", Toast.LENGTH_SHORT).show();
                        }
                    }
                }
            }
        ...
        switch (v.getId()){
            case R.id.backButton_new:
                for(DynamicMenuModel menuModel : AppConstants.getParentlist()) {
                    if (menuModel.getMenucode().equalsIgnoreCase("mnu_open_saving_account")) {
                        Intent intent = new Intent(OpenSavingsAccountActivity.this, MenuActivity.class);
                ...
                Intent intent = new Intent(OpenSavingsAccountActivity.this, SaveMenu.class);
                intent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                startActivity(intent);
                finish();
                break;
    ...
    private void submit() {
       ...
       String identificationno =et_identificationno.getText().toString();
       String date=tv_investmentdate.getText().toString();*/
        ...
        if (product_spi.getSelectedItem().equals(getResources().getString(R.string.SelectProductType))) {
            ...
            if (TrustMethods.isSimAvailable(getApplicationContext()) && TrustMethods.isSimVerified(OpenSavingsAccountActivity.this)) {
                if (NetworkUtil.getConnectivityStatus(OpenSavingsAccountActivity.this)) {
                    new AsyncTasksave(OpenSavingsAccountActivity.this, productid, remarks,needCheque).execute();
                } else {
                    Toast.makeText(this, getResources().getString(R.string.error_check_internet), Toast.LENGTH_SHORT).show();
                }
            } else {
    ...
    private class AsyncTasksave extends AsyncTask<Void, Void, String> {
        private ProgressDialog pDialog;
        String error;
        Context ctx;
        String response;
        ...
        public AsyncTasksave(Context ctx, String productid, String remarks, String needCheque) {
            this.error = "";
            this.ctx = ctx;
        ...
        protected String doInBackground(Void... params) {
            try {
                String url = TrustURL.GetOpenAccount(AppConstants.getCLIENTID(),productid,remarks, "Saving Account",needCheque);
                if (!url.equals("")) {
        ...
        protected void onPostExecute(String value) {
            ...
            Intent i=new Intent(ctx,SuccessSplashActivity.class);
            i.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP | Intent.FLAG_ACTIVITY_CLEAR_TOP);
            i.putExtra("Title",getResources().getString(R.string.ThankYou2));
            i.putExtra("Activity", "OpenSavingsAccountActivity");
            startActivity(i);
        }
    }
    private class AsyncTaskGetSavinglookups extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... params) {
            try {
                ...
                if (responseCode.equals("1")) {
                    savingSchemes = jsonResponse.getJSONObject("response").getJSONArray("Table1");
            ...
            Identification_spi.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                ...
                public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                    if(position != 0){
                        ...
                        Toast.makeText(ctx, "Ans"+identificationid, Toast.LENGTH_SHORT).show();
            ...
            branch_spi.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                ...
                public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                    if(position != 0){
                        ...
                        Toast.makeText(ctx, "Ans"+branchid, Toast.LENGTH_SHORT).show();
